...
```apex
Date latestDate = nlib_List.collect(<list of contacts>).pluck('CreatedDate').max();
``` 
...
As mentioned above, the ```collect``` method returns a new instance for the given array.  Creating a new collection is as simple as:

```apex
nlib_List collection = nlib_List.collect(new Contact[]{});
```
...
Returns an array of nlib_Lists.
...
Compares this list to another array.  Returns ```true``` if they are the same.
...
Compares this list to another array. Returns ```true``` if they are different.
...
Returns the first element in the collection or ```null``` if the list is empty.
